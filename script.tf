...
resource "azurerm_public_ip" "publicip" {
  ...
  tags = {
    turma = "2022-MBA-AS-04"
    disciplina = "Infrastructure and Cloud Architecture"
    alunos = "Marlon Tiedt, Bruna Santos Nicolau de Oliveira e Josué Medeiros Azevedo Lima"
  }
}

resource "azurerm_network_interface" "mtsys-nic" {
  name                = "mtsys-nic"
  location            = azurerm_resource_group.as04infra.location
  resource_group_name = azurerm_resource_group.as04infra.name
...
resource "azurerm_network_interface_security_group_association" "ng-nic-assoc" {
  network_interface_id      = azurerm_network_interface.mtsys-nic.id
...
resource "azurerm_linux_virtual_machine" "vm-terraform" {
  ...
  network_interface_ids = [
    azurerm_network_interface.mtsys-nic.id
  ]
